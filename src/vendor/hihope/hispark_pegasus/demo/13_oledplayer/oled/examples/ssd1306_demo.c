...
void TestDrawChinese1(void)
{
    ...
    ssd1306_SetCursor(NUMBER_64, 0);
    ssd1306_DrawRegion(NUMBER_64, 0, headSize[0], headSize[1], headData, sizeof(headData), headSize[0]);
...
void TestDrawChinese2(void)
{
    ...
    ssd1306_SetCursor(0, NUMBER_64 - NUMBER_8);
    ssd1306_DrawString("gitee.com", Font_6x8, White);

    ssd1306_SetCursor(0, NUMBER_64 - NUMBER_8 * NUMBER_2);
...
void TestShowChars(FontDef font, uint8_t w, uint8_t h)
{
    ...
    uint8_t x = 0, y = 0;
    for (uint8_t c = 1; c < NUMBER_128; c++) {
        if (isprint(c)) {
...
{
    IoSetFunc(HI_IO_NAME_GPIO_0, HI_IO_FUNC_GPIO_0_I2C1_SDA);
    IoSetFunc(HI_IO_NAME_GPIO_1, HI_IO_FUNC_GPIO_1_I2C1_SCL);
    ...
    while (count--) {
        TestDrawChinese1();
        osDelay(NUMBER_5*NUMBER_100);

        TestDrawChinese2();
        osDelay(NUMBER_5*NUMBER_100);

        TestShowChars(Font_6x8, NUMBER_6, NUMBER_8);
        osDelay(NUMBER_5*NUMBER_100);

        TestShowChars(Font_7x10, NUMBER_7, NUMBER_10);
        osDelay(NUMBER_5*NUMBER_100);
