...
* 步骤二：修改applications/sample/wifi-iot/app/目录下的BUILD.gn，在features字段中添加i2s_demo:i2cDemo。注：第一个i2s_demo指的是需要编译的工程目录，第二个指的是applications/sample/wifi-iot/app/i2s_demo/BUILD.gn文件中的静态库，名称为i2cDemo。

```c
import("//build/lite/config/component/lite_component.gni")

lite_component("app") {
  features = [ "i2s_demo:i2cDemo", ]
}
```
