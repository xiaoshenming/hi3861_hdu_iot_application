...
* <font color='RedOrange '>注意：下方的\\\192.168.56.101\share中的192.168.56.101，是本文ubuntu的IP地址，每个人的ubuntu的IP地址可能不一样，这里需要注意。</font>

<img src="C:/Users/Admin/Desktop/hi3861_hdu_iot_application_cy/pic/image-20220915115717505.png" style="zoom:80%;" />

* 这样Ubuntu的家目录就能够在Windows的磁盘下面显示了。这样您就可以方便的进行Windows和Ubuntu之间的文件共享了。

    

<img src="pic/image-20220915163147810.png" style="zoom:67%;" />
...
- **<font color='RedOrange '>注意：如果您的Ubuntu重启之后，需要再次启动dcoker编译环境的话</font>**，需要执行下面的命令
...
* 在docker编译环境的终端，执行下面的命令，进行代码的下载。
...
* 执行下面的命令，进入Ubuntu的 /home/hispark目录下,然后添加权限，"code"为步骤四映射文件夹，用户可以自己指定。
...
* 步骤2：修改./device/hisilicon/hispark_pegasus/sdk_liteos/build/config/usr_config.mk文件，由于这个文件是开启Hi3861功能文件，如I2C,I2S，PWM等等，具体修改如下（这里我只开启了I2C,PWM,UART等，用户根据自己实际情况去修改）。
...
* 步骤3：双击刚下载好的CH341SER.EXE驱动，进入安装界面，点击安装按钮即可，驱动安装成功后，再点击确定按钮。
...
* 通过串口工具可以看到 helloworld！已经打印出来，说明我们编写的helloworld程序已经成功在开发板上运行起来了。
...
* 步骤1：复制./vendor/hisilicon/hispark_pegasus/demo/led_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。
...
* 3.硬件配置：将J7跳帽跳至C2A-GPIO 0，J5跳帽跳至C2B-GPIO 12，将MOTOR_EN拨码开关由ON拨到OFF；硬件搭建如下图所示：
    ...
    ![]($pic/image-20221017145245652.png)
...
* 步骤1：复制./vendor/hisilicon/hispark_pegasus/demo/encoder_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。
...
* 3.将MOTOR_EN拨码开关由ON拨到OFF，硬件搭建如下图所示：
    ...
    <img src="pic/image-20220916094819674.png" style="zoom:25%;" />

### 2.3.3 实验流程

* 步骤1：./vendor/hisilicon/hispark_pegasus/demo/tricolor_lamp_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。
...
- 步骤4：修改.device/soc/hisilicon/hi3861v100/sdk_liteos/build/config/usr_config.mk文件。在这个配置文件中打开I2C驱动宏。搜索字段CONFIG_I2C_SUPPORT ，并打开I2C。配置如下：
...
* 3.将MOTOR_EN拨码开关由ON拨到OFF，硬件搭建如下图所示：
    ...
    <img src="pic/image-20220916110114188.png" style="zoom: 50%;" />

### 2.4.3 实验流程

* 步骤1：./vendor/hisilicon/hispark_pegasus/demo/button_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。
...
* 步骤1：./vendor/hisilicon/hispark_pegasus/demo/hcsr04_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。
...
* 3.硬件配置：将MOTOR_EN拨码开关由OFF拨到ON，硬件搭建如下图所示：
    ...
    <img src="pic/image-20220916113435131.png" style="zoom: 33%;" />
...
* 步骤1：./vendor/hisilicon/hispark_pegasus/demo/motor_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。
...
* 步骤1：./vendor/hisilicon/hispark_pegasus/demo/sg92r_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。
...
* 步骤1：./vendor/hisilicon/hispark_pegasus/demo/cw2015_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。

* 步骤2：修改applications/sample/wifi-iot/app/目录下的BUILD.gn，在features字段中添加cw2015_demo:cw2015_control。注：第一个cw2015_demo指的是需要编译的工程目录，第二个cw2015_control指的是applications/sample/wifi-iot/app/cw2015_demo/BUILD.gn文件中的静态库，名称为cw2015_control。
...
* 步骤3：I2C通信原理：I2C通信中，主机通过时钟线SCL发送时钟信号，通过数据线SDA发送数据（包括从机地址、指令、数据包等），在发送完一帧数据后，需要等待从机的响应，才能继续发送下一帧数据，因此I2C属于同步通信。
...
* 步骤4：修改.device/hisilicon/hispark_pegasus/sdk_liteos/build/config/usr_config.mk文件。在这个配置文件中打开I2C驱动宏。搜索字段CONFIG_I2C_SUPPORT ，并打开I2C。配置如下：
...
* 步骤1：./vendor/hisilicon/hispark_pegasus/demo/nfc_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。

* 步骤2：修改applications/sample/wifi-iot/app/目录下的BUILD.gn，在features字段中添加nfc_demo: nfc_control。注：第一个nfc_demo指的是需要编译的工程目录，第二个nfc_control指的是applications/sample/wifi-iot/app/ nfc_demo/BUILD.gn文件中的静态库，名称为nfc_control。
...
* 步骤3：原理：FM11NT081D使用I2C通信，INT_NFC在IO扩展芯片IO0_6上（默认为高，低有效），CS_NFC在IO扩展芯片I01_6上，当CS_NFC脚为低时，正常通信，手机靠近NFC之后，NFC产生中断，使IO扩展芯片IO0_6被拉低，触发IO扩展芯片中断，实现了I2C中断触发。
...
* 软件烧录成功后，使用串口工具连接好小车的串口，按一下开发板的RESET按键复位开发板，当我们使用手机靠近小车的NFC模块时，会拉起手机的淘宝应用，同时可以看到串口打印出read nfc的字样，说明我们使用I2C中断触发的功能，实现对NFC驱动的实验成功。串口打印信息如下所示。
...
* 步骤1：./vendor/hisilicon/hispark_pegasus/demo/i2c_bus_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。

* 步骤2：修改applications/sample/wifi-iot/app/目录下的BUILD.gn，在features字段中添加i2c_bus_demo: i2c_bus_demo。注：第一个i2c_bus_demo指的是需要编译的工程目录，第二个i2c_bus_demo指的是applications/sample/wifi-iot/app/ i2c_bus_demo/BUILD.gn文件中的静态库，名称为i2c_bus_demo。
...
* 步骤4：原理：I2C通信中，主机通过时钟线SCL发送时钟信号，通过数据线SDA发送数据（包括从机地址、指令、数据包等），在发送完一帧数据后，需要等待从机的响应，才能继续发送下一帧数据，因此I2C属于同步通信。一个I2C主设备可以与多个I2C从设备通信。
...
* 步骤1：./vendor/hisilicon/hispark_pegasus/demo/lth1550_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。

* 步骤2：修改applications/sample/wifi-iot/app/目录下的BUILD.gn，在features字段中添加lth1550_demo:lth1550_control。注：第一个lth1550_demo指的是需要编译的工程目录，第二个lth1550_control指的是applications/sample/wifi-iot/app/ lth1550_demo/BUILD.gn文件中的静态库，名称为lth1550_control。
...
步骤4：原理：ADC即模拟-数字转换器，可以将连续变化的模拟信号转换为数字信号，进而使用数字电路进行处理，称之为数字信号处理。
...
* 软件烧录成功后，使用串口工具连接好小车的串口，按一下开发板的RESET按键复位开发板，当我们将红色对管置于黑色或者白色的背景下，串口打印出来的ADC数据有明显的变化，说明我们使用主芯片的ADC功能，实现对红外对管驱动的实验成功。注意：如果要使用小车的红外对管，需要使用跳线帽选择J5和J6的2、3脚，不然无法检测到数据。
...
* 步骤1：./vendor/hisilicon/hispark_pegasus/demo/uart_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。
...
* 步骤3：UART通信原理：数据从一个设备UART的发送引脚（Tx） 流向另一个设备UART的接收（Rx） 引脚。
...
- 步骤4：修改.device/hisilicon/hispark_pegasus/sdk_liteos/build/config/usr_config.mk文件。在这个配置文件中打开UART驱动宏。搜索字段CONFIG_UART1_SUPPORT ，并打开I2C。配置如下：
...
#### 2.12.3.2 DisconnectWithHotspot()
...
* 步骤1：./vendor/hisilicon/hispark_pegasus/demo/wifi_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。

* 步骤2：修改applications/sample/wifi-iot/app/ wifi_demo/目录下的BUILD.gn，如果使用Hi3861V100 STA模式，将"wifi_starter.c"和"wifi_hotspot_demo.c"屏蔽，如果使用Hi3861V100 AP模式，将"wifi_connecter.c"和"wifi_connect_demo.c"屏蔽（注意：只能同时使用一种模式）。
...
* 步骤3：修改applications/sample/wifi-iot/app/目录下的BUILD.gn，在features字段中添加wifi_demo: wifi_control。注：第一个wifi_demo指的是需要编译的工程目录，第二个wifi_control指的是applications/sample/wifi-iot/app/wifi_demo/BUILD.gn文件中的静态库，名称为wifi_control。
...
* 步骤4：Hi3861V100作为STA模式(参考步骤2修改文件实现demo功能)，使用开发板连接路由器或者手机热点，可以在wifi_connect_demo.c修改热点名称和密码（注意：只支持4G网络）本demo wifi名：“H”，密码：“12345678”。核心代码如下所示。
```c
1 /*
2  * Copyright (c) 2022 HiSilicon (Shanghai) Technologies CO., LIMITED.
3  * Licensed under the Apache License, Version 2.0 (the "License");
...
* 如果Hi3861作为STA模式，打开SSCOM串口工具，Hi3861连接WiFi成功后出现如下打印：

    ![](pic/image-20220919161056658.png)

* 如果Hi3861作为AP模式，打开SSCOM串口工具，手机可以搜索到HiSpark-AP，然后连接，同时串口出现如下打印：
...
* 步骤1：./vendor/hisilicon/hispark_pegasus/demo/thread_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。
...
* 步骤4：在docker的命令行终端，执行下面的命令，进行代码的编译。
...
* 步骤1：./vendor/hisilicon/hispark_pegasus/demo/semaphore_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。

* 步骤2：修改applications/sample/wifi-iot/app/目录下的BUILD.gn，在features字段中添加semaphore_demo:semaphore_demo。注：第一个semaphore_demo指的是需要编译的工程目录，第二个semaphore_demo指的是applications/sample/wifi-iot/app/ semaphore_demo /BUILD.gn文件中的静态库，名称为semaphore_demo。
...
* 步骤3：原理：信号量是一种实现任务间通信的机制，实现任务之间同步或临界资源的互斥访问。在多任务操作系统中，不同的任务之间需要同步运行，信号量功能可以为用户提供这方面的支持。当任务申请(Pend)信号量时，如果申请成功，则信号量的计数值递减，如若申请失败，则挂起在该信号量的等待任务队列上，一旦有任务释放该信号量，则等待任务队列中的任务被唤醒开始执行。代码如下：
...
* 软件烧录成功后，使用串口工具连接好小车的串口，按一下开发板的RESET按键复位开发板，可以从串口工具看到，当人行道线程获取到交通灯信号量时，会输出 Pedestrians pass and vehicles stop，此时人行道进入倒计时5秒的工作状态，此时机动车道处于等待状态，5秒后，释放交通灯信号量，此时机动车道线程获取到交通灯信号量，会输出Vehicles pass and pedestrians stop，此时机动车道进入倒计时10秒的工作状态，此时人行道处于等待状态，10秒后释放交通灯信号量，然后再次进入循环，说明我们通过使用信号量，在不同线程中访问共享资源，来模拟交通灯的实验成功。
...
* 步骤1：./vendor/hisilicon/hispark_pegasus/demo/timer_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。

* 步骤2：修改applications/sample/wifi-iot/app/目录下的BUILD.gn，在features字段中添加timer_demo: timer_demo。注：第一个timer_demo指的是需要编译的工程目录，第二个timer_demo指的是applications/sample/wifi-iot/app/ timer_demo /BUILD.gn文件中的静态库，名称为timer_demo。
...
* 步骤3：原理：软件定时器使用了系统的一个队列和一个任务资源，软件定时器的触发遵循队列规则，先进先出。定时时间短的定时器总是比定时时间长的靠近队列头，满足优先被触发的准则。
...
* 软件烧录成功后，使用串口工具连接好小车的串口，按一下开发板的RESET按键复位开发板，可以从串口工具看到，首先会创建一个定时器，然后开启定时器，当100个时钟周期后，就会调用一次宠物喂食机的投喂功能，投喂5次后，就会停止定时器，然后删除定时器，当出现如下的打印信息，就说明使用定时器，来实现宠物喂食机的定时投喂的实验成功。
...
* 步骤1：./vendor/hisilicon/hispark_pegasus/demo/interrupt_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。

* 步骤2：修改applications/sample/wifi-iot/app/目录下的BUILD.gn，在features字段中添加interrupt_demo:interrupt_demo。注：第一个interrupt_demo指的是需要编译的工程目录，第二个指的是applications/sample/wifi-iot/app/interrupt_demo/BUILD.gn文件中的静态库，名称为interrupt_demo。
...
* 步骤3：原理：中断是指CPU暂停执行当前程序，转而执行新程序的过程。中断相关的硬件可以划分为3类：
...
* 软件烧录成功后，使用串口工具连接好小车的串口，按一下开发板的RESET按键复位开发板，可以从串口工具看到。
...
* 完整代码参考：ultrasonic_car_demo。本demo实现了S1按键菜单选项栏，进入对应界面修改小车前进速度，左转右转速度，舵机转动角度，超声波距离临界值，方向角角度，通过S2,S3增加每个模块对应参数值。在主界面时，通过S2,S3按键实现小车开启停止超声波避障。当小车正前方距离障碍物较近时，小车停止，舵机左右转动，获取左右距离，哪边距离大小车往哪边转向，转弯的角度超过90°。具体请参考工程代码。
...
49     car_drive.LeftForward = 10; // 10 左轮前进速度
...
57     car_drive.rightangle = 500; // 舵机右转90度
58 }
59 
...
65     ssd1306_printf("TL:%d, RT:%d", car_drive.TurnLeft, car_drive.TurnRight);
...
75             ssd1306_printf("TL:%d, TR:%d", car_drive.TurnLeft, car_drive.TurnRight);
...
* 3.接线方式：将MOTOR_EN拨码开关由OFF拨到ON，插上J5 GPIO 12 - PWM3 ,J6 GPIO 7 - TRIG，同时插上J13GPIO7 - RIN，J12 GPIO12 - LIN；硬件搭建如下图所示：
    ...
    ![](pic/image-20220922150521392.png) <img src="$pic/image-20221019183734026.png" style="zoom: 67%;" />
     ...
     ![]($pic/image-20221019183717643.png)
 2. 如果使用IO扩展板左右红外管测距接收，右红外管需要J13跳帽跳至RIN_IO、RIN;左红外管测需要J12跳帽跳至LIN_IO、LIN;
...
* 完整代码参考：trace_demo。本demo 实现了S1按键菜单选项栏，进入对应界面修改小车前进速度，左转右转速度，通过S2,S3增加每个模块对应参数值。在主界面时，通过S2,S3按键实现小车开启停止寻迹。小车左轮在黑线时，小车左大灯亮白光，右轮转动，小车右轮在黑线时，小车右大灯亮白灯，小车左轮转动，小车两边都不在黑线时，小车前进。具体请参考工程代码。
...
47     car_drive.TurnLeft = 30; // 30 左转弯右轮速度
...
58     ssd1306_printf("TL:%d, RT:%d", car_drive.TurnLeft, car_drive.TurnRight);
...
83             ssd1306_printf("TL:%d, RT:%d", car_drive.TurnLeft, car_drive.TurnRight);
...
129             car_drive.TurnLeft += ((button_pressed) ? -1 : 1);
130             ssd1306_printf("TurnLeft=%d", car_drive.TurnRight);
131             break;
132         case MODE_SET_TURN_RIGHT:
133             car_drive.TurnRight += ((button_pressed) ? -1 : 1);
...
237                 car_left(car_drive.TurnRight);
238                 LeftLED();
239             } else if ((m_left_value == 1) && (m_right_value == 2)) { // 右偏，向左转 2代表右边在白线的状态
240                 car_right(car_drive.TurnLeft);
241                 RightLed();
...
* 如果使用扩展IO口左右红外管测距
    ...
    完整代码参考：trace_ex_demo。本demo 实现了S1按键菜单选项栏，进入对应界面修改小车前进速度，左转右转速度，通过S2,S3增加每个模块对应参数值。在主界面时，通过S2,S3按键实现小车开启停止寻迹。小车左轮在黑线时，小车左大灯亮白光，右轮转动，小车右轮在黑线时，小车右大灯亮白灯，小车左轮转动，小车两边都不在黑线时，小车前进。具体请参考工程代码。
...
23 #include "iot_errno.h"
24 #include "iot_gpio_ex.h"
25 #include "pca9555.h"
26 #include "motor_control.h"
27 #include "hi_time.h"
28 #include "trace_control.h"
29 
30 CAR_DRIVE car_drive = { 0 };
31 ENUM_MODE g_mode = MODE_ON_OFF;
32 
33 int g_CarStarted = 0;
34 
35 #define MASK_BUTTON1        (0x10)
36 #define MASK_BUTTON2        (0x08)
37 #define MASK_BUTTON3        (0x04)
38 #define MASK_TRACE1         (0x01)
39 #define MASK_TRACE2         (0x02)
40 
41 void init_ctrl_algo(void)
42 {
43     (void)memset(car_drive, 0, sizeof(CAR_DRIVE));
44     car_drive.LeftForward = 10; // 10 左轮前进速度
45     car_drive.RightForward = 10; // 10 右轮前进速度
46     car_drive.TurnLeft = 30; // 30 左转弯右轮速度
47     car_drive.TurnRight = 30; // 30 右转弯左轮速度
48 }
49 
50 void init_oled_mode(void)
51 {
52     g_mode = MODE_ON_OFF;
53     ssd1306_ClearOLED();
54     ssd1306_printf("LF:%d, RF:%d", car_drive.LeftForward, car_drive.RightForward);
55     ssd1306_printf("TL:%d, RT:%d", car_drive.TurnLeft, car_drive.TurnRight);
56 }
57 
58 void LeftLED(void)
59 {
60     PCA_WriteReg(PCA9555_REG_OUT1, LEFT_LED); /* IO1 012345低电平 */
61 }
62 
63 void RightLed(void)
64 {
65     PCA_WriteReg(PCA9555_REG_OUT1, RIGHT_LED); /* IO1 012345低电平 */
66 }
67 
68 void LedOff(void)
69 {
70     PCA_WriteReg(PCA9555_REG_OUT1, LED_OFF); /* IO1 012345低电平 */
71 }
72 
73 void ButtonDesplay(ENUM_MODE mode)
74 {
75     switch (mode) {
76         case MODE_ON_OFF:
77             ssd1306_printf("LF:%d, RF:%d", car_drive.LeftForward, car_drive.RightForward);
78             ssd1306_printf("TL:%d, TR:%d", car_drive.TurnLeft, car_drive.TurnRight);
79             break;
80         case MODE_SET_LEFT_FORWARD:
81             ssd1306_printf("Set LForward=%d", car_drive.LeftForward);
82             break;
83         case MODE_SET_RIGHT_FORWARD:
84             ssd1306_printf("Set RForward=%d", car_drive.RightForward);
85             break;
86         case MODE_SET_TURN_LEFT:
87             ssd1306_printf("Set TurnLeft=%d", car_drive.TurnRight);
88             break;
89         case MODE_SET_TURN_RIGHT:
90             ssd1306_printf("Set TurnRight=%d", car_drive.TurnLeft);
91             break;
92         default:
93             break;
94     }
95 }
96 
97 void ButtonSet(ENUM_MODE mode, bool button_pressed)
98 {
99     printf("mode = %d\r\n", mode);
100     switch (mode) {
101         case MODE_ON_OFF:
102             g_CarStarted = !g_CarStarted;
103             ssd1306_ClearOLED();
104             printf("g_CarStarted = %d\r\n", g_CarStarted);
105             ssd1306_printf(g_CarStarted ? "start" : "stop");
106             break;
107         case MODE_SET_LEFT_FORWARD:
108             car_drive.LeftForward += ((button_pressed) ? -1 : 1);
109             ssd1306_printf("LeftForward=%d", car_drive.LeftForward);
110             break;
111         case MODE_SET_RIGHT_FORWARD:
112             car_drive.RightForward += (button_pressed ? -1 : 1);
113             ssd1306_printf("RightForward=%d", car_drive.RightForward);
114             break;
115         case MODE_SET_TURN_LEFT:
116             car_drive.TurnLeft += ((button_pressed) ? -1 : 1);
117             ssd1306_printf("TurnLeft=%d", car_drive.TurnRight);
118             break;
119         case MODE_SET_TURN_RIGHT:
120             car_drive.TurnRight += ((button_pressed) ? -1 : 1);
121             ssd1306_printf("TurnRight=%d", car_drive.TurnLeft);
122             break;
123         default:
124             break;
125     }
126 }
127 
128 void ButtonPressProc(uint8_t ext_io_val)
129 {
130     static uint8_t ext_io_val_d = 0xFF;
131     uint8_t diff;
132     static int time_stamp = 0;
133     bool button1_pressed, button2_pressed, button3_pressed, trace1_pressed, trace2_pressed;
134     diff = ext_io_val ^ ext_io_val_d;
135     button1_pressed = ((diff & MASK_BUTTON1) && ((ext_io_val & MASK_BUTTON1) == 0)) ? true : false;
136     button2_pressed = ((diff & MASK_BUTTON2) && ((ext_io_val & MASK_BUTTON2) == 0)) ? true : false;
137     button3_pressed = ((diff & MASK_BUTTON3) && ((ext_io_val & MASK_BUTTON3) == 0)) ? true : false;
138     trace1_pressed = ((diff & MASK_TRACE1) && ((ext_io_val & MASK_TRACE1) == 0)) ? true : false;
139     trace2_pressed = ((diff & MASK_TRACE2) && ((ext_io_val & MASK_TRACE2) == 0)) ? true : false;
140     ssd1306_ClearOLED();
141     if (button1_pressed) {
142         g_mode = (g_mode >= (MODE_END - 1)) ? 0 : (g_mode + 1);
143         ButtonDesplay(g_mode);
144     } else if (button2_pressed || button3_pressed) {
145         ButtonSet(g_mode, button2_pressed);
146     }
147     if ((hi_get_milli_seconds() - time_stamp) > 100 && g_CarStarted) { // 100ms判断一次
148         time_stamp = hi_get_milli_seconds();
149         if (trace1_pressed && !trace2_pressed) {
150             car_left(car_drive.TurnRight);
151             LeftLED();
152         } else if (!trace1_pressed & trace2_pressed) {
153             car_right(car_drive.TurnLeft);
154             RightLed();
155         } else if (!trace2_pressed && !trace2_pressed) {
156             car_forward(car_drive.LeftForward, car_drive.RightForward);
157             LedOff();
158         } else {
159             car_stop();
160         }
161     } else if (!g_CarStarted) {
162         car_stop();
163     }
164     ext_io_val_d = ext_io_val;
165 }
166 
167 void TraceDemoTask(void)
168 {
169     InitPCA9555();
170     GA12N20Init();
171     TaskMsleep(100); // 等待100ms初始化完成
172     init_ctrl_algo();
173     init_oled_mode();
174     PCA_RegisterEventProcFunc(ButtonPressProc);
175 }
176 
177 void TraceSampleEntry(void)
178 {
179     osThreadAttr_t attr;
180     attr.name = "TraceSampleEntry";
181     attr.attr_bits = 0U;
182     attr.cb_mem = NULL;
183     attr.cb_size = 0U;
184     attr.stack_mem = NULL;
185     attr.stack_size = 1024 * 5; /* 堆栈大小为1024*5 */
186     attr.priority = osPriorityNormal;
187 
188     if (osThreadNew((osThreadFunc_t)TraceDemoTask, NULL, &attr) == NULL) {
189         printf("[TraceSampleEntry] Failed to create TraceDemoTask!\n");
190     }
191 }
192 APP_FEATURE_INIT(TraceSampleEntry);
```
## 4.3 IMU小车控制实验
### 4.3.1 实验目的
...
* 完整代码参考：imu_square_demo。本demo实现了S1按键菜单选项栏，进入对应界面修改小车前进速度，左转速度，方向角角度，小车前进时间，通过S2,S3增加每个模块对应参数值。在主界面时，通过S2,S3按键实现小车开启停止小车走正方形。参考代码如下。
...
```flow
st=>start: 上电运行
op=>operation: IMU模块和其他模块初始化
cond=>condition: 检测LSM6是否正常
op1=>operation: 读取姿态信息
cond1=>condition: 小车是否倒下
op2=>operation: 检测转速
op3=>operation: 计算PID
cond2=>condition: 超出最大转速
op4=>operation: 输出左右轮PWM和转向
op5=>operation: 限幅输出

st->op->cond
cond(yes)->op1->cond1
cond1(no,bottom)->op2->op3->cond2
cond1(yes)->cond
cond2(no,bottom)->op4
cond2(yes)->op5
op5->op4
...
* 完整代码参考：balance_car_demo。本demo实现了S1按键菜单选项栏，进入对应界面修改小车俯仰角，kp系数，kd系数，通过S2,S3增加每个模块对应参数值。在主界面时，通过S2,S3按键实现小车开启停止平衡车。具体请参考工程代码。
...
* 步骤1：./vendor/hisilicon/hispark_pegasus/demo/histreaming_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。
...
* 步骤4：首先控制台选择 “北京四”，然后点击 “产品”；

    ![](pic/image-20220920152227956.png)

*  步骤5：选择 “创建产品”，然后弹出对话框，根据提示完善信息，点击 “确定”，出现创建产品成功代表产品创建完成。
...
* 步骤11：./vendor/hisilicon/hispark_pegasus/demo/oc_demo文件夹复制到./applications/sample/wifi-iot/app/目录下。
...
* 步骤12：修改applications/sample/wifi-iot/app/oc_demo/目录下iot_config.h中CONFIG_AP_SSID，CONFIG_AP_PWD为WiFi名称和WiFi密码以及CONFIG_DEVICE_ID和CONFIG_DEVICE_PWD。CONFIG_DEVICE_ID和CONFIG_DEVICE_PWD为下面图片红色框框内容。

```c
/* Please modify the ssid and pwd for the own */
#define CONFIG_AP_SSID  "x" // WIFI SSID
#define CONFIG_AP_PWD "xxxxxxx" // WIFI PWD
...
* 步骤13：./applications/sample/wifi-iot/app/oc_demo/app_demo_iot.c中下列字段参数与自定义创建模型属性及服务保持一致(用户自己定义)，使用参考案例不用修改。
...
* 步骤14：在华为云如下界面点击总览，可以看到平台接入地址，点击进去之后可以看到设备接入MQTT字样如下图所示，复制“a162297e00.iot-mqtts.cn-north-4.myhuaweicloud.com”字段，将该字段写到./applications/sample/wifi-iot/app/oc_demo/iot_main.c中CN_IOT_SERVER字段中。具体修改如下图所示。
...
* 步骤15：修改源码./applications/sample/wifi-iot/app下的BUILD.gn文件，在features字段中增加索引，使目标模块参与编译。features字段指定业务模块的路径和目标,features字段配置如下:
...
* 步骤16：修改.device/hisilicon/hispark_pegasus/sdk_liteos/build/config/usr_config.mk文件。在这个配置文件中打开I2C,PWM驱动宏。搜索字段CONFIG_I2C_SUPPORT ，并打开I2C。配置如下：
...
* 步骤17：MQTT云端互联使用的paho.mqtt，目前已提供适配鸿蒙系统的paho.mqtt，在"./device/hisilicon/hispark_pegasus/sdk_liteos/third_party/paho.mqtt.c/include/mqtt"目录下，通过mqtt连接华为云核心代码实现如下：
...
- 步骤19 :烧录成功后，等待连接云端成功，串口出现如下打印网页端显示设备在线即可。

    ![](pic/image-20220924162413633.png)

    ![](pic/image-20220924162450791.png)

- 步骤20：点击查看，同时按不同按键控制小车红色灯亮灭，可以看到最新数据上报情况。

    ![](pic/image-20220924162556473.png)

- 步骤21：在命令栏选择同步命令下发，选择创建的命令，然后根据代码设置的下发参数，发送对应指令，可以看到小车红色LED亮灭。
