...
static int Byte2HexStr(unsigned char* bufin, int len, char* bufout)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        bufout[BUF_OUT_LEN_2 * i] = (tmp_h > IOT_HEX_TO_STRING_1) ? (tmp_h - IOT_HEX_TO_STRING_2 + 'a') : (tmp_h + '0');
        bufout[BUF_OUT_LEN_2 * i + BUF_OUT_LEN_1] =
            (tmp_l > IOT_HEX_TO_STRING_1) ? (tmp_l - IOT_HEX_TO_STRING_2 + 'a') : (tmp_l + '0');
    }
