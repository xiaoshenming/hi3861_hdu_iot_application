...
static const char* hisignallingLevelNames[] = { "TRACE", "DEBUG", "INFO", "WARN", "ERROR", "FATAL" };
...
static hi_u32 HisignallingDataPackage(HisignallingProtocalType* buf, hi_u32 len, hi_u8* hisignallingDataBuf)
{
    ...
    memcpy_s(hisignallingDataBuf, HISGNALLING_MSG_FRAME_HEADER_LEN, buf->frameHeader, HISGNALLING_MSG_FRAME_HEADER_LEN);
    memcpy_s(&hisignallingDataBuf[HISGNALLING_MSG_FRAME_HEADER_LEN], len, buf->hisignallingMsgBuf, len);
    memcpy_s(&hisignallingDataBuf[HISGNALLING_MSG_FRAME_HEADER_LEN + len], HISIGNALLING_MSG_HEADER_LEN,
...
HisignallingErrorType HisignallingMsgReceive(hi_u8* buf, hi_u32 len)
{
    ...
    if (len > HISGNALLING_MSG_CRC32_LEN) {
        ...
        if (((hi_u8)((crcCheckReceived & 0xff000000) >> RIGHT_MOVE_24_BIT) != buf[len - 4]) && /* 4: addr offset */
            ...
            HISIGNALLING_LOG_ERROR("<Hi3861>:crc_4=0x%x, crc_3=0x%x, crc_2=0x%x, crc_1=0x%x", buf[len - BUF_LEN_4],
                                   buf[len - BUF_LEN_3], buf[len - BUF_LEN_2], buf[len - BUF_LEN_1]); /* 4,3,2,1 addr offset */
