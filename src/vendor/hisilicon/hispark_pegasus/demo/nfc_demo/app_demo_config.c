...
unsigned char C08iNfcI2cWrite(unsigned char regHigh8bitCmd, unsigned char regLow8bitCmd, unsigned char* dataBuff,
                              unsigned char len)
{
    ...
    IoTI2cWrite(id, C081_NFC_ADDR & 0xFE, C081nfcI2cWriteCmdAddr.sendBuf, C081nfcI2cWriteCmdAddr.sendLen);

    return 0;
}

/* 写寄存器 */
unsigned int WriteFifoReg(unsigned char regHigh8bitCmd, unsigned char regLow8bitCmd, unsigned char dataBuff)
{
    IotI2cIdx id = IOT_I2C_IDX_0;
    IotI2cData c081nfcI2cWriteCmdAddr = { 0 };
...
unsigned int Fm11ReadEep(unsigned char* dataBuff, unsigned short ReadAddr, unsigned short len)
{
    WriteRead((unsigned char)((ReadAddr & 0xFF00) >> 8),       /* 8: right move 8 bit */
              (unsigned char)(ReadAddr & 0x00FF), dataBuff, 2, /* 2: read lenght */
              len);
...
void SetApdu(unsigned char* statusOk, unsigned char* statusWord2, const unsigned char* ndefCapabilityContainer,
             const unsigned char* ndefId)
{
    if (fm327Fifo[P1] == 0x00) { /* 0x00:  FIFO P1 cmd */
        if ((fm327Fifo[LC] == FIFO_LC_CMD) && (memcmp(ndefCapabilityContainer, fm327Fifo + DATA, fm327Fifo[LC]) == 0)) {
            Fm11WriteFifo(statusOk, 3);      /* 3: statusOk lenght */
            Fm11WriteReg(RF_TXEN_REG, 0x55); /* 0x55: TX reg cmd */
            currentFile = CC_FILE;
        } else if ((fm327Fifo[LC] == FIFO_LC_CMD) && (memcmp(ndefId, fm327Fifo + DATA, fm327Fifo[LC]) == 0)) {
            Fm11WriteFifo(statusOk, 3);      /* 3: statusOk lenght */
...
void SelectApdu(unsigned char* statusOk, unsigned char* statusWord, unsigned char* statusWord2,
                const unsigned char* ndefCapabilityContainer, const unsigned char* ndefId)
{
    unsigned char xlen;
    unsigned char xbuf[256] = { 0 };

    if (fm327Fifo[INS] == 0xA4) { /* 0xA4:  FIFO INS cmd */
        SetApdu(statusOk, statusWord2, ndefCapabilityContainer, ndefId);
        ...
        if (currentFile == CC_FILE) {
            ...
            (void)memcpy_s(&xbuf[1],                                                           /* 1: xbuf lenght */
                           NDEF_FILE_LEN, &ndefFile[0] + (fm327Fifo[P1] << 8) + fm327Fifo[P2], /* 8: left move 8 bit */
                           fm327Fifo[LC]);
        ...
        (void)memcpy_s(ndefFile + (fm327Fifo[P1] << 8) + fm327Fifo[P2], /* 8: left move 8 bit */
                       NDEF_FILE_LEN, fm327Fifo + DATA, fm327Fifo[LC]);
        Fm11WriteFifo(statusOk, 3);      /* 3: statusOk lenght */
        Fm11WriteReg(RF_TXEN_REG, 0x55); /* 0x55: TX reg cmd */
    } else {
...
void Fm11T4t(void)
{
    ...
    if (crcErr) {
        ...
        SelectApdu(statusOk, statusWord, statusWord2, ndefCapabilityContainer, ndefId);
    }
}
