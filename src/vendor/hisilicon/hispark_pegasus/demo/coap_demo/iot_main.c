...
static int MsgRcvCallBack(unsigned char* context, char* topic, int topicLen, MQTTClient_message* message)
{
    ...
    if (buf != NULL) {
        ...
        ret = memcpy_s(buf, bufSize, topic, topicLenght);
        if (ret != EOK) {
            return;
        }
        buf[topicLenght] = '\0';
        ...
        ret = memcpy_s(buf, bufSize, message->payload, message->payloadlen);
        if (ret != EOK) {
            return;
        }
...
static hi_void MainEntryProcess(hi_void)
{
    ...
    ret = snprintf_s(clientID,
                     ...
                     CONFIG_DEVICE_ID, CN_EVENT_TIME);
    if (ret < 0) {
        printf("string is null\r\n");
    }
    userID = CONFIG_DEVICE_ID;
    if (CONFIG_DEVICE_PWD != DEVICE_PWD_DEFAULT) {
...
int IotSendMsg(int qos, const char* topic, const char* payload)
{
    ...
    if (buf != NULL) {
        ...
        ret = memcpy_s(buf, bufSize, topic, strlen(topic));
        if (ret != EOK) {
            return;
        }
        buf[strlen(topic)] = '\0';
        ...
        ret = memcpy_s(buf, bufSize, payload, strlen(payload));
        if (ret != EOK) {
            return;
        }
