 ...
 */
static unsigned int Ath20CheckAndInit(unsigned char initCmd, unsigned char initHighByte, unsigned char initLowByte)
{
...
void* AppDemoAht20(char* param)
{
    ...
    while (1) {
        if (ret == IOT_SUCCESS) {
            ...
            while (IOT_SUCCESS !=
                   Ath20CheckAndInit(AHT_DEVICE_INIT_CMD, AHT_DEVICE_PARAM_INIT_HIGH, AHT_DEVICE_PARAM_LOW_BYTE)) {
