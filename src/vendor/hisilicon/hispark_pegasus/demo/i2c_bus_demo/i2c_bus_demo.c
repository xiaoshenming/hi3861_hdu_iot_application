...
void OnFuncKeyPressed(const char *arg)
{
    (void) arg;
    g_buttonState = 1;
}
...
// NFC模块task
void NFCTask(int* arg)
{
    NFCInit(); // 初始化NFC模块
}

void LSM6DSTask(int* arg)
{
    (void) arg;
    GetLSM6DS();
}
...
void I2CBusInit(void)
{
    ...
    uint8_t write[2] = {0X12, 0x30};
    // 使能IO扩展芯片的中断寄存器
    IoTI2cWrite(0, 0XD4, write, 2);
