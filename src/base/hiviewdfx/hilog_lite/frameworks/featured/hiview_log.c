...
#define bool _Bool
static int HILOG_TRUE = 1;
static int HILOG_FALSE = 0;
...
#else
    if (g_hilogFd == -1) {
        g_hilogFd = open(HILOG_DRIVER, O_WRONLY | O_CLOEXEC);
    }
    if (g_hilogFd == -1) {
        return 0;
    }
