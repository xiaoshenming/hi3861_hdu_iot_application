# hi3861_openharmony_lite

![platform](https://img.shields.io/badge/platform-Hi3861V100-brightgreen?logo=huawei&style=for-the-badge) ![license](https://img.shields.io/badge/license-Apache-brightgreen?style=for-the-badge) ![OS](https://img.shields.io/badge/OS-OpenHarmony-brightgreen?style=for-the-badge) ![star](https://img.shields.io/badge/dynamic/json?style=for-the-badge&color=brightgreen&logo=Gitee&label=stars&url=https://gitee.com/api/v5/repos/pineapple-sea/hispark_t1_balance&query=$.stargazers_count) ![watch](https://img.shields.io/badge/dynamic/json?style=for-the-badge&color=brightgreen&logo=Gitee&label=watch&url=https://gitee.com/api/v5/repos/pineapple-sea/hispark_t1_balance&query=$.watchers_count) ![fork](https://img.shields.io/badge/dynamic/json?style=for-the-badge&color=brightgreen&logo=Gitee&label=fork&url=https://gitee.com/api/v5/repos/pineapple-sea/hispark_t1_balance&query=$.forks_count)



## 介绍
欢迎使用<物联网技术与应用>课程代码仓, 这是一个学习使用Hi3861V100开发OpenHarmony物联网应用的项目.

## 硬件说明
这是一个嵌入式的软件项目, 所以你需要有一块Hi3861V100的开发板. 如果你只是想学习使用Hi3861开发WiFi物联网应用, 那么基本上任何一块基于Hi3861V100的开发板都是可以的. 如果你需要使用这个项目中的全部外设实验用例, 例如陀螺仪, NFC, 马达控制, 平衡车等等, 那么你需要配合一块HiSpark T1开发板使用.
...
## 开发环境
这个项目的开发环境必须使用Windows+Linux虚拟机. 我们推荐的虚拟机系统配置为VirtualBox 6.0 + Ubuntu20.04, 虚拟机内存2G以上, 虚拟机硬盘推荐20G以上. 

有了虚拟机后, 你可以参考doc目录下的教程手动安装所需的Linux软件, 搭建所有的软件编译环境. 如果你觉得自己从头搭建环境对你来说太复杂, 或者你担心新装软件会与原本系统里的一些软件冲突, 或者你只是想小试一下OpenHarmony的开发体验, 或者你单纯就是懒的话!😶, 我们推荐你使用我们已经封装好的Docker, 因为它是如此的方便. 你只要按照以下的指导一步一步输入命令就可以:
...
2. 拉取镜像到本地
    ...
    ```
3. 新建一个容器命名为openharmony, 并映射用户目录~到容器内目录/home/hispark, 映射容器端口22为外部端口2222
    ```bash
...
把编译后的镜像文件copy到Windows中(通过samba或ssh), 然后运行hiburn.exe, 将镜像下载到板上运行. 这里我还是推荐你用命令行的方式运行: 在windows中建立这样一个脚本, 并命名为例如fast_burn.bat之类的名字, 复制以下内容, 并将大括号{}部分替换为你的实际信息, 修改并保存.
...
如果你对项目中的代码或者文档存在疑问, 欢迎在Issues中提出你的问题(别忘了先在FAQ中看一看是否已经有答案了😎). 如果你自己解决了一个了不起的问题, 非常欢迎你把问题和解决方法发到Issues里, 如果你看到别人的问题而你正好有答案, 也欢迎你帮助解答其他人的问题, 毕竟"授人玫瑰手有余香"嘛.
